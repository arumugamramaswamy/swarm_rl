...
class CustomSimpleEnv(SimpleEnv):
    ...
    def state(self):
        states = tuple(self.scenario.observation(self.world.agents[self._index_map[agent]], self.world) for agent in self.possible_agents)
        return states
...
class raw_env(CustomSimpleEnv):
    ...
    """
    def __init__(self, N=3, shuffle=False, local_ratio=0.5, max_cycles=25, continuous_actions=False, reward_only_single_agent=False, reward_agent_for_closest_landmark=False):
        assert 0. <= local_ratio <= 1., "local_ratio is a proportion. Must be between 0 and 1."
        scenario = CustomScenario(shuffle=shuffle, reward_only_single_agent=reward_only_single_agent, reward_agent_for_closest_landmark=reward_agent_for_closest_landmark)
