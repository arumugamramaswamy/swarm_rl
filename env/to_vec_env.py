...
class MarkovVectorEnv(VecEnv):
    ...
    def concat_obs(self, obs_dict):
        """
        Stuff to note:
        In the case of non-array observation spaces such as Dict or Tuple, where
        different sub-spaces may have different shapes, the sub-observations are vectors (of dimension n).
        """
        obs = {key: [] for key in self.observation_space}

        for _, agent in enumerate(self.par_env.possible_agents):
            ...
            for key in obs:
                obs[key].append(obs_dict[agent][key])
    ...
    def reset(self):
        _observations = self.par_env.reset()
    ...
    # below methods are mandatory
    def env_is_wrapped(self, _):
        """
    ...
    def get_attr(self, attr_name, _=None):
        return getattr(self.par_env, attr_name)

    def set_attr(self, attr_name, value, _=None):
        return setattr(self.par_env, attr_name, value)

    def env_method(self, method_name, *method_args, _=None, **method_kwargs):
