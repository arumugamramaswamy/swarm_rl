...
class CrossAttention(nn.Module):
    ...
    def __init__(self, input_size, embedding_size, query_size, num_heads) -> None:
        ...
        self._cross_attn1 = CustomAttention(
            embedding_size, embedding_size, embedding_size + query_size, num_heads
        )
    ...
    def forward(self, obs1, obs2, query):
        ...
        m1, _ = self._cross_attn1(
            c1, th.cat([th.sum(c2, dim=-2).unsqueeze(-2), query], dim=-1)
        )
        m2, _ = self._cross_attn2(
